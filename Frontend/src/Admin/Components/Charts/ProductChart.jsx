...
const ProductChart = ({ products, review, cart, wishlist, paymentData }) => {
  ...
  const cartData = [
    {
      ...
      "Quantity in cart": cart.filter(
        (prod) => prod.productId.type === "cloths"
      ).length,
    },
    {
      name: "Shoes",
      "Quantity in cart": cart.filter((prod) => prod.productId.type === "shoe")
        .length,
    },
    {
      name: "Electronics",
      "Quantity in cart": cart.filter(
        (prod) => prod.productId.type === "electronics"
      ).length,
    },
    {
      name: "Books",
      "Quantity in cart": cart.filter((prod) => prod.productId.type === "book")
        .length,
    },
    {
      name: "Jewelry",
      "Quantity in cart": cart.filter(
        (prod) => prod.productId.type === "jewelry"
      ).length,
    },
  ];
  ...
  const wishlistData = [
    {
      name: "Cloths",
      "Quantity in wishlist": wishlist.filter(
        (prod) => prod.productId.type === "cloths"
      ).length,
    },
    {
  ...
  const groupedData = paymentData
    ...
    .reduce((acc, item) => {
      ...
      if (index !== -1) {
        acc[index].totalAmount += totalAmount;
      } else {
        acc.push({ month: month, totalAmount: totalAmount });
      }
      return acc;
    }, []);
