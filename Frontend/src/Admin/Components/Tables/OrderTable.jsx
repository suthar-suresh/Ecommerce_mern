...
const orderStatusConfirm = async (id) => {
  let authToken = localStorage.getItem("Authorization");
  const response = await axios.post(
    process.env.REACT_APP_ORDER_STATUS`${id}`,
    {
      userDetails: JSON.stringify({ orderStatus: "Apporved" }),
    },
    {
      headers: {
                  ...
                  <TableRow>
                    <TableCell>
                      <IconButton
                        ...
                        onClick={() =>
                          setOpenOrderId(
                            openOrderId === order._id ? "" : order._id
                          )
                        }
                      >
                        {<MdKeyboardArrowDown />}
                    ...
                    <TableCell>
                      <Link to={`user/${order.user}`}>{order.totalAmount}</Link>
                    </TableCell>
                    <TableCell>
                      <Link to={`user/${order.user}`}>
                              ...
                              {order.productData?.map((product) => (
                                <TableRow key={product._id}>
                                  <TableCell align="left">
                                    <Link
                                      to={`/admin/home/product/${product.productId.type}/${product.productId._id}`}
                                    >
